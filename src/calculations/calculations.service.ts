...
import {
  ServiceMaterial,
} from '../service-materials/entities/service-material.entity';
...
export class CalculationsService {
  constructor(
    ...
    private readonly serviceMaterialRepository:
  ...
  async create(
    ...
    if (
      !createCalculationDto.services?.length
    ) {
    ...
    const client =
      await this.clientRepository.findOne({
        where: {
          id:
            createCalculationDto.client_id,
        },
      });
    ...
    return this.dataSource.transaction(
      async (
        ...
        const savedCalculation =
          await calculationRepository.save(
            calculation,
          );
  ...
  /**
   * Genera el número de factura usando:
   *
   * FAC-AAAAMMDD-001
   *
   * El consecutivo empieza nuevamente
   * desde 001 cada día.
   */
